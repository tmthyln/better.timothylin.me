<template>
  <div class="has-text-centered">
    <div class="mb-5">
      <h1 class="title is-1">{{ currentWord }}</h1>
    </div>
...
const { data: wordData, execute: fetchRandomWord } = useFetch('/api/random-word', { immediate: true }).json<{ word: string }>()
const currentWord = computed(() => wordData.value?.word ?? '')

const minutes = ref(2)
const seconds = ref(0)
const initialMinutes = ref(2)
